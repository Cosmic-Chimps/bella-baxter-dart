...
void main() {
  ...
  print(
      'EXTERNAL_API_KEY          : ${env['EXTERNAL_API_KEY'] ?? '(not set)'}');
  ...
  print(
      'ConnectionStrings__Postgres: ${env['ConnectionStrings__Postgres'] ?? '(not set)'}');
  print(
      'APP_CONFIG                : ${env['APP_CONFIG']?.replaceAll(r'\\\"', '"').replaceAll(r'\"', '"') ?? '(not set)'}');
